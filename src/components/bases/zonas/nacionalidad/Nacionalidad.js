    ...
    methods: {
        getAll(pag) {
            ...
            this.$proxies._zonasProxi
                .getAllNacionalidad(pag, 6) //EJECUTA LOS PROXIS QUE INYECTA AXIOS
                .then((x) => {
        ...
        gets(id) {
            this.__limpiarCampos();
            this.$proxies._zonasProxi.getIdNacionalidad(id)
                .then((x) => {
                    this.model = x.data;
                }).catch(() => {
                    console.log("Error")
        ...
        __eliminar(idn) {
            ...
            if (confirm('ESTA SEGURO QUE QUIERE ELIMINAR?')) {
                this.$proxies._zonasProxi
                    .removeNacionalidad(idn) //EJECUTA LOS PROXIS QUE INYECTA AXIOS
                    .then(() => {
                        this.$notify({
                            group: "global",
