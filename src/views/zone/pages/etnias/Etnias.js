  ...
  methods: {
    ...
    save() {
      this.$validate().then(success => {
        ...
        if (this.model._id) {
          ...
          this.ifLoad = true;
          this.model.nombre = this.model.nombre.trim();
          this.$proxies._zonasProxi.createEtnias(this.model)
            .then(() => {
              this.close();
              this.ifLoad = false;
              this.getAll(this.pagina, 8);
    ...
    gets() {
      ...
      if (isArray === 1) {
        this.isCarga = true;
        this.openModal();
        this.$proxies._zonasProxi.getIdEtnias(this.isSelecUsers[0])
          .then((x) => {
            this.model = x.data;
            this.isCarga = false;
          }).catch(() => {
    ...
    dialogDelete() {
      ...
      if (this.isSelecUsers.length > 0) {
        this.$proxies._zonasProxi
          .removeEtnias(this.isSelecUsers)
          .then(() => {
    ...
    dialogState() {
      if (this.isSelecUsers.length > 0) {
        ...
        this.$proxies._zonasProxi
          .activateEtnias(this.isSelecUsers, state)
          .then(() => {
            this.iseliminaddo = false;
            this.isSelecUsers = [];
            this.getAll(this.pagina, 8);
